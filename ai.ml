...
let get_ai_action (gs: game_state) (bs : battle_status) =
  ...
  (List.iter (fun (x: (float * Types.move)) -> print_endline ((snd x).name ^
    " " ^ (string_of_float (fst x)) )) s_p_list);
  (print_endline ("switch score: " ^ switch_poca.name ^
    " " ^ (string_of_float switch_score)));
  (List.iter (fun (x:pocamon) -> print_endline x.name)
    active_player.pocamon_list);
